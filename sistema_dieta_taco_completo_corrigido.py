...
def gerar_dieta_personalizada():
    ...
    # Pergunta qual dia gerar
    print(f"\nDias disponíveis: {', '.join(dias_semana)}")
    dia_escolhido = input("Qual dia da semana gerar a dieta? ").lower().strip()

    if dia_escolhido not in dias_semana:
        print(f"Dia '{dia_escolhido}' não encontrado. Usando 'segunda' como padrão.")
        dia_escolhido = 'segunda'

    calorias_diarias = calorias_por_dia[dia_escolhido]
    ...
    print(f"\n=== DIETA PARA {dia_escolhido.upper()} - PACIENTE TIPO {tipo_paciente} ===")
    print(f"Calorias do dia: {calorias_diarias:.0f} kcal")
    ...
    # Resumo final
    print("\n" + "="*80)
    print(f"RESUMO NUTRICIONAL - {dia_escolhido.upper()}")
    print("="*80)
